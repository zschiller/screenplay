 ...
 * the gesture never derives geometry itself), and on commit a
 ...
 *
 * Ported so far (#535 migration order): the **gap-resize** path (#538) and the
 * **reorder** path (#542) — in-flow reorder of a Member within its Group plus
 * the meta-key pop-out into a new Group. The remaining gestures land in order
 * (move/merge → marquee → device-resize), each adding its own arm to the
 * `GestureState` / `GestureEvent` / `GestureIntent` unions.
 */
...
/**
 * The single active gesture, or `idle` at rest. New arms (move, marquee,
 * resize) join this union as each gesture is ported.
 */
 ...
 * `move` carries the live cursor (and meta-key state for gestures that read it);
 * `metaChange` flips the meta-key between moves (a key press with no pointer
 * motion); `release` commits; `cancel` aborts with no intent.
 */
 ...
 * `reorder` floats/pops the dragged Member. Both `null` outside their gesture.
 */
 ...
 * through local selection state. New arms (`moveBy`, `mergeGroups`,
 * `resizeLayer`, `marqueeSelect`, …) join as each gesture is ported.
 *
 * - `setGroupGap` / `reorderMember` / `popOutToNewGroup`: canvas writes.
...
export function reduceGesture(
  ...
  switch (event.type) {
    ...
    case "move": {
      if (state.kind === "gap") {
        ...
        return {
          ...
          preview: { gapOverride: { groupId: state.ctx.groupId, gap }, reorder: null },
        }
      }
    ...
    case "metaChange": {
      // A key press/release with no pointer motion: flip the pop-out preview at
      // the last cursor. The in-flow reindex waits for the next move (matching
      // the original key listener, which only toggled the popped flag).
      if (state.kind === "reorder") {
...
function previewFor(state: GestureState): GesturePreview {
  ...
  if (state.kind === "reorder") {
    return { gapOverride: null, reorder: reorderPreviewOf(state) }
  }
