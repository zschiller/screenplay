 ...
 *   3. Add a width/height resolver case in `lib/artboard-layout.ts`.
 *   4. Build the canvas component (mirror `DocumentLayer`).
 ...
 *
 * `<T>` is the concrete data shape for the kind (e.g. `ArtboardData`,
 * `DocumentLayerData`).
 */
export interface LayerKindDescriptor<T = unknown> {
   ...
   *  sidebar — e.g. a route badge for artboards. */
