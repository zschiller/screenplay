 ...
 * screenplay side and to an input shape the prototype produces. The agent
 * declares knobs by writing `useKnob({...})` calls in the prototype's code;
 * the helper file (see `lib/skills/knobs/helper.tsx`) posts the declarations
 * up to the canvas via `screenplay:knobs-declared`, and value updates flow
 * back via `screenplay:knob-values`.
 */
