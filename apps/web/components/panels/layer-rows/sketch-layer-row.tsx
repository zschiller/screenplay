...
import { sketchLayerKind } from "@/lib/layer-kinds/sketch-layer"
...
export function SketchRow({
  ...
  onSelect,
  onActivate,
}: LayerRowProps<SketchLayerData>) {
  const Icon = sketchLayerKind.Icon
  if (variant === "flat") {
    return (
      <SidebarMenuButton
      ...
      >
        ...
        <span className="truncate">{sketchLayerKind.getLabel(item)}</span>
      </SidebarMenuButton>
    )
  }
  return (
...
export function SketchRowMenu({
  item,
  isSub,
  onRename,
  onRemove,
}: LayerRowMenuProps<SketchLayerData>) {
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent side="right" align="start" className="w-48">
        <DropdownMenuItem
          onClick={() => {
            const newTitle = prompt("Rename sketch", item.title || "Untitled")
